...
class DirectoryChecker
{
    ...
    public static function getDirectoryStatus($path, int $mode = 0777, $redirectFile = null): bool|string
    {
        ...
        if (!@\is_dir($path)) {
            ...
            $path_status .= "<button class='submit' onClick='this.form.submit();'>" . _DC_WGGALLERY_CREATETHEDIR . '</button>';
            $path_status .= '</form>';
        } elseif (@is_writable($path)) {
            $path_status = "<img src='$pathIcon16/1.png' >";
            $path_status .= "$path (" . _DC_WGGALLERY_AVAILABLE . ') ';
            ...
            if ($currentMode != decoct($mode)) {
                ...
                $path_status .= "<input type='hidden' name='path' value='$path'>";
                $path_status .= "<input type='hidden' name='redirect' value='$redirectFile'>";
                $path_status .= "<button class='submit' onClick='this.form.submit();'>" . _DC_WGGALLERY_SETMPERM . '</button>';
                $path_status .= '</form>';
            }
        } else {
            $currentMode = mb_substr(decoct(fileperms($path)), 2);
